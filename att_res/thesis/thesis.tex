...
In recent years there has been a growing search for efficient, subquadratic alternatives to Transformer, to make training and inference cheaper. The main approaches are to use a lighter variation of attention, such as DeepSeek Sparse Attention, or the use a linear architecture, such as RWKV or DeltaNet. 
While Reservoir Computing is a class of architectures capable of state of the art prediction for time-series or speech recognition tasks, or more in general tasks that trains on continuous data, it struggles with discrete data, such as tokens. 
