...
def _reservoir_scan_ip(
    ...
    """
    Compiled leaky-integration reservoir scan with optional output feedback,
    applying trainable node-wise gain (ip_a) and bias (ip_b).
    """
    h = h0
    states = []
    for t in range(x.shape[1]):
        ...
        if W_fb is not None:
            pre_act = pre_act + F.linear(h, W_fb)
        h_new = torch.tanh(ip_a * pre_act + ip_b)
...
class AERC(nn.Module):
    ...
    def set_phase(self, phase: int) -> None:
        ...
        if phase == 1:
            ...
            self.ip_a.requires_grad_(False)
            self.ip_b.requires_grad_(False)
        elif phase == 2:
            ...
            self.ip_a.requires_grad_(True)
            self.ip_b.requires_grad_(True)
        else:
...
def pretrain_reservoir_ip(
    ...
    print("IP pre-training completed. Model gain (ip_a) and bias (ip_b) initialized.")
